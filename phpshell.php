...
//        setcookie(session_name(), '', time()-42000, '/');
   ...
   * replay attach where one uses the back button to re-authenticate
   * using the old POST data since the server wont know the session
   * then.*/
//    session_destroy();
}
...
if ($_SESSION['authenticated']) {  
  ...
  if (!empty($command)) {
    ...
    if (preg_match('^[[:blank:]]*cd[[:blank:]]*$', $command)) {
      ...
      /* The command is not an internal command, so we execute it
       * after changing the directory and save the output. */
      chdir($_SESSION['cwd']);
...
<?php
if (!$_SESSION['authenticated']) {
  /* Genereate a new nounce every time we preent the login page.  This
   * binds each login to a unique hit on the server and prevents the
   * simple replay attack where one uses the back button in the
   * browser to replay the POST data from a login. */
